...
- 뷰어와 카메라 페이지 모두 영상 위에 캔버스 오버레이를 유지하며, 좌표는 0~1 범위로 정규화하여 송수신합니다.
- 판서 좌표는 Socket.IO 이벤트 `annotation:point`로 양방향 전달되며, 수신 시 캔버스에 즉시 렌더링합니다.
- 카메라 페이지는 팔레트·판서 토글 UI가 포함되어 있으며, 스트림과 별도로 오버레이에서 선을 그릴 수 있습니다.
- 아직 영상 스트림에 판서 내용을 합성하거나 10초 만료/삭제 같은 라이프사이클 관리는 구현되지 않았습니다. 세부 남은 항목은 [`TODO.md`](./TODO.md)에 정리되어 있습니다.
